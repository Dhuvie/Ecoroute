...
> **EcoRoute — ML-Driven Logistics Optimization Platform**
>
> - Architected and built a production-grade, full-stack logistics optimization platform that reduces fuel consumption by **~18%** across large delivery networks by combining ML fuel prediction, multi-algorithm route optimization (Dijkstra, A*, OR-Tools CVRP), and an interactive analytics dashboard.
> - Designed an end-to-end ML pipeline comparing **6 regressors** (Linear Regression, Random Forest, Gradient Boosting, XGBoost, LightGBM, CatBoost) with **Optuna** hyperparameter tuning and 5-fold cross-validation, achieving **R²=0.9883** and **MAE=15.89 L** with CatBoost on a 50,000-record synthetic dataset.
> - Engineered a **FastAPI** backend with 20+ REST endpoints, SQLAlchemy 2.0 ORM, Pydantic v2 schemas, and service-layer architecture (fuel prediction, routing, fleet optimization, weather, traffic, analytics) — backed by an interactive **OpenAPI 3.0** spec.
> - Implemented the **Capacitated Vehicle Routing Problem** solver using **OR-Tools** with a greedy capacity-balanced fallback, optimizing assignments across vehicle capacity, fuel efficiency, distance, and deadlines.
> - Built a modern **React 18 + TypeScript + TailwindCSS** frontend with 7 pages (dashboard, vehicles, deliveries, route optimizer, fleet optimizer, analytics, ML model insights), featuring **Leaflet** interactive maps and **Plotly** charts in a dark theme.
> - Generated a physically-grounded **50,000-record synthetic dataset** modeling fuel consumption as a function of distance, mass, elevation, traffic, weather, road type, and vehicle characteristics.
> - Containerized the full stack with **multi-stage Dockerfiles** and orchestrated via **docker-compose** (FastAPI + React/nginx + PostgreSQL), with **GitHub Actions** CI running backend tests, ML pipeline validation, frontend type-checking, and Docker builds on every push.
